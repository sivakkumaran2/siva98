...
export function GradientPicker({
    ...
    <Popover>
      <PopoverTrigger asChild>
        <Button
          variant={'outline'}
          className={cn(
            'grid w-full gap-2 md:grid-cols-2 lg:grid-cols-2 ',
            !background && 'text-muted-foreground',
            className
          )}
        >
          <div className="w-full flex items-center gap-2 text-xs md:text-sm">
            {background ? (
              <div
      ...
      <PopoverContent className="w-64">
        <Tabs defaultValue={defaultTab} className="w-full">
          <TabsList className="w-full mb-4">
            <TabsTrigger className="flex-1 text-xs md:text-sm" value="solid">
              Solid
            </TabsTrigger>
            <TabsTrigger className="flex-1 text-xs md:text-sm" value="gradient">
              Gradient
            </TabsTrigger>
            {/* <TabsTrigger className="flex-1 text-xs md:text-sm" value="image">
              Image
            </TabsTrigger> */}
          </TabsList>
  ...
  id="custom"
  value={background}
  className="col-span-2 mt-4 text-xs md:text-sm "
  style={{ height: '20px' }}
...
const GradientButton = ({ 
  ...
  return (
    <div
      ...
      style={{ background }}
    >
      <div className="bg-popover/80 rounded-md p-1 text-xs md:text-sm text-center">
        {children}
      </div>
    </div>
  )
